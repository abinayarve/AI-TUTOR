...
class RAGEngine:
    ...
    # -----------------------------------------------------------------------
    # Streamlit / app-layer adapters
    #
    # NOTE:
    # The rest of this class is transport-agnostic (works with plain file
    # paths). The two methods below are thin adapters used by app.py so the
    # UI layer never has to know about temp-file handling or the shape of
    # retrieve_grounded()'s return value. Historically app.py called methods
    # with these exact names that did not exist on this class, which meant
    # every file upload silently failed and fell back to ungrounded
    # "general knowledge" context. These adapters fix that.
    # -----------------------------------------------------------------------
    ...
    def process_and_index_file(
        ...
        """
        Accept a Streamlit ``UploadedFile`` (or a plain path/bytes-like
        object) and index it.

        ``clear_existing`` defaults to True so that starting a new lesson
        from a new upload does not keep mixing in chunks from a previous,
        unrelated document.
        """
        ...
        try:
            ...
            # Streamlit UploadedFile / any file-like object with .read().
            name = getattr(uploaded_file, "name", "uploaded_material")
            ...
            tmp_dir = Path(tempfile.mkdtemp(prefix="ai_teacher_upload_"))
            tmp_path = tmp_dir / name if name else tmp_dir / f"upload{suffix}"
            ...
            return {
                "success": False,
                "status": "error",
                "message": f"Could not index uploaded file: {exc}",
            }
    ...
    def retrieve_context(
        ...
        """
        Return grounded context text for a query, optionally narrowed to a
        specific chapter/section/topic the learner asked to focus on.

        ``focus`` is combined into the retrieval query (not just the
        display string) so the vector search itself is scoped toward the
        requested chapter/section rather than the whole document.
        """
        ...
        if focus:
            ...
            if focus:
                search_query = (
                    f"{search_query} — focus specifically on: {focus}"
                    if search_query
                    else focus
                )

        result = self.retrieve_grounded(
            search_query,
            top_k=top_k,
        )
